...
class RegisteredDbRealm {
    ...
    def authenticate(authToken) {
        ...
        // exception.
        def user = RegisteredUser.findByUsername(username)
        if (!user) {
        ...
        // in the database.
        def account = new SimpleAccount(username, user.passwordHash, "RegisteredDbRealm")
    ...
    def hasRole(principal, roleName) {
        def roles = RegisteredUser.withCriteria {
            roles {
                eq("name", roleName)
            }
    ...
    def hasAllRoles(principal, roles) {
        def r = RegisteredUser.withCriteria {
            roles {
                'in'("name", roles)
            }
            eq("username", principal)
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        def user = RegisteredUser.findByUsername(principal)
        ...
        def results = RegisteredUser.executeQuery("select distinct p from RegisteredUser as user join user.roles as role join role.permissions as p where user.username = '$principal'")
